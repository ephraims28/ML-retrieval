...
class pyPamtra(object):
  ...
  def __init__(self):
    ...
    self.nmlSet["radar_pnoise0"]= -30 # mean value for BArrow MMCR during ISDAC
    ...
    self.nmlSet["radar_k2"]= 0.711  #WCR =0.711 KAZR = 0.88 #0.93 # dielectric constant |K|² (always for liquid water by convention) for the radar equation
    ...
    self.nmlSet["radar_attenuation"]=  "bottom-up" #! "bottom-up" or "top-down" #add bottom up
    self.nmlSet["radar_polarisation"]=  "VV" #! comma separated
    self.nmlSet["radar_use_wider_peak"]=  False #
    self.nmlSet["radar_integration_time"] =  0.0495 #KAZR = 3.7 WCR=0.0495 #1.4 # MMCR Barrow during ISDAC
    self.nmlSet["radar_fwhr_beamwidth_deg"] = 0.63 #KAZR = 0.31 #WCR =0.63 #0.31 # full width haalf radiation beam width MMCR Barrow
  ...
  def readPamtraSounding(self, ncInputFile, q):
    """
    read lay or lev pamtra profile from file. Descriptot file must be defined before
    ...
    nlayers = (f2['alt'][:]>0).sum()
    validAltIndexes = [
        index for index in range(len(f2['alt'][:]))
        if f2['alt'][:][index] > 0
    ]
    
    #print(validAltIndexes)
    
    # TODO, use number of lines from csv file
    self.p["ngridx"], self.p["ngridy"], self.p["max_nlyrs"], self.p["noutlevels"] = [1,1,nlayers-1,2]
    ...
    for xx in range(self._shape2D[0]):
      for yy in range(self._shape2D[1]):
        ...
        year,month,day,hour,minute, self.p["nlyrs"][xx,yy], self.p["model_i"][xx,yy], self.p["model_j"][xx,yy] = [dtime.year,dtime.month,dtime.day,dtime.hour,dtime.minute,nlayers-1,1,1]
        
        self.p["unixtime"][xx,yy] = calendar.timegm(datetime.datetime(year = int(year), month = int(month), day = int(day), hour = int(hour), minute = int(minute), second = 0).timetuple())
        
        
        # Top of atmosphere and bottom of atmosphere
        self.p["obs_height"][xx,yy,:] = [83300, 10]
        
        #TODO customize for csv

        self.p["lat"][xx,yy], self.p["lon"][xx,yy], lfrac,self.p["wind10u"][xx,yy],self.p["wind10v"][xx,yy],self.p["groundtemp"][xx,yy],self.p["hgt_lev"][xx,yy,0]  = [f2['lat'][0], f2['lon'][0], 0., f2['u_wind'][0],  f2['v_wind'][0], f2['tdry'][0]+273.15,   f2['alt'][0],     ]
        ...
        #self.p["iwv"][xx,yy] = 11.7
        importTimeIdxOffset = 0
        for zz in range(self.p["nlyrs"][xx,yy]+1):
          
          self.p["hgt_lev"][xx,yy,zz] = f2['alt'][validAltIndexes[zz]] #dataLineNew[24]
            
          self.p["press_lev"][xx,yy,zz] = float(f2['pres'][validAltIndexes[zz]])*100
          self.p["temp_lev"][xx,yy,zz] = float(f2['tdry'][validAltIndexes[zz]])+273.15
          self.p["relhum_lev"][xx,yy,zz] = float(f2['rh'][validAltIndexes[zz]])
        
          if zz<self.p["nlyrs"][xx,yy]:
              self.p["hgt"][xx,yy,zz] = self.p["hgt_lev"][xx,yy,zz]
              self.p["press"][xx,yy,zz] = self.p["press_lev"][xx,yy,zz]
              self.p["temp"][xx,yy,zz] = self.p["temp_lev"][xx,yy,zz]
              self.p["relhum"][xx,yy,zz] = self.p["relhum_lev"][xx,yy,zz]
    
          #for hydrometeors it's always layers:
          if zz < self.p["nlyrs"][xx,yy]:
              for hh in range(self.df.nhydro):
                #print(hh)
                #print(self.df.data["moment_in"][hh])
                if self.df.data["moment_in"][hh] == 0:
                  pass #nothing to read...
                elif self.df.data["moment_in"][hh] == 1:
                  self.p["hydro_n"][xx,yy,zz,hh] = dataLine.pop(0)
                  ...
                  self.p["hydro_q"][xx,yy,zz,hh] = q
                elif self.df.data["moment_in"][hh] == 12:
                  self.p["hydro_n"][xx,yy,zz,hh] = dataLine.pop(0)
                  self.p["hydro_reff"][xx,yy,zz,hh] = dataLine.pop(0)
                elif self.df.data["moment_in"][hh] == 13:
                  self.p["hydro_n"][xx,yy,zz,hh] = 1000000
                  self.p["hydro_q"][xx,yy,zz,hh] = q
                elif self.df.data["moment_in"][hh] == 23:
                  self.p["hydro_reff"][xx,yy,zz,hh] = dataLine.pop(0)
                  self.p["hydro_q"][xx,yy,zz,hh] = dataLine.pop(0)
  ...
  def createProfile(self,**kwargs):
    ...
    if "hydro_q" in list(kwargs.keys()):
      print(self.df.nhydro)
      print(kwargs["hydro_q"].shape[-1])
    ...
    for environment, preset in [["lat",50.938056],["lon",6.956944],["wind10u",0],["wind10v",0],["groundtemp",np.nan],["sfc_salinity",33.],["sfc_slf",1.],["sfc_sif",0.]]:
      if environment not in list(kwargs.keys()):
        ...
        if type(kwargs[environment]) in (int,np.int32,np.int64,float,np.float32,np.float64):
          ...
          print(environment)
          print(kwargs[environment].shape)
...
#            newP[x,y] = np.exp(np.interp(new_hgt[x,y],old_hgt[x,y],np.log(self.p[key][x,y])))
  ...
  def runParallelPamtraRandom(self,freqs,select, pp_local_workers="auto",pp_deltaF=1,pp_deltaX=0,pp_deltaY = 0,checkData=True,timeout=None):
    '''
    run Pamtra parallel from python. Populates result dictionary 'r'.
...
#          if (self.p["obs_height"][i,j,k] < tmp_hgt_lev[-1]) and (self.p["obs_height"][i,j,k] > tmp_hgt_lev[0]):
  ...
  def _prepareResults(self):
    ...
    if self.nmlSet["active"]:
        ...
        self.r["Ze"] = np.array([missingNumber])
    ...
    if self.nmlSet["active"]:
        ...
        self.r["radar_snr"] = np.array([missingNumber])
        self.r["radar_moments"] = np.array([missingNumber])
        self.r["radar_slopes"] = np.array([missingNumber])
        self.r["radar_edges"] = np.array([missingNumber])
        self.r["radar_quality"] = np.array([missingNumber])
        self.r["radar_vel"] = np.array([missingNumber])
