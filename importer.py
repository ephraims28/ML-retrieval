...
def downsample_3d_array(array,res, grid):
    """
    Downsamples a 3D array using 2D average pooling.
    ...
    """
    factor = int(np.ceil(res // grid))
    x_dim, y_dim, z_dim = array.shape
    new_x_dim = x_dim // factor
    new_y_dim = y_dim // factor

    downsampled_array = np.zeros((new_x_dim, new_y_dim, z_dim))

    for z in range(z_dim):
        for x in range(new_x_dim):
            for y in range(new_y_dim):
                downsampled_array[x, y, z] = np.mean(array[x*factor:(x+1)*factor, y*factor:(y+1)*factor, z])

    
    return downsampled_array

def downsample_2d_array(array,res, grid):
    """
    Downsamples a 3D array using 2D average pooling.
    ...
    downsampled_array = np.zeros((new_x_dim, new_y_dim))


    for x in range(new_x_dim):
        for y in range(new_y_dim):
            downsampled_array[x, y] = np.mean(array[x*factor:(x+1)*factor, y*factor:(y+1)*factor])

    
    return downsampled_array

def readWrfCOMBLESat(fWind, fCloud, descriptorFile,resolution):
  """
  ...
  variables = ['XLONG', 'XLAT', 'U10', 'V10', 'TSK', 'PSFC', 'T', 'P', 'PB', 'QVAPOR', 'QCLOUD', 'PH', 'PHB','RE_CLOUD','QICE','RE_ICE','QSNOW','RE_SNOW','QGRAUP','QRAIN','QNRAIN','QNCLOUD']
    
  data = dict()

  ncFileWind = netCDF4.Dataset(fWind,"r")
  ncFileCloud = netCDF4.Dataset(fCloud,"r")

  for var in variables:
        print(var)
        if var in ['XLONG', 'XLAT','U10', 'V10', 'TSK']:
            data[var] = ncFileWind.variables[var][0,800:1000, 1400:1600 ].T    
            #data[var] = ncFileWind.variables[var][0,6300:6800,1692:1693 ].T 
        if var in ['T', 'P', 'PB', 'PH', 'PHB']:
            data[var] = ncFileWind.variables[var][0,:,800:1000, 1400:1600 ].T    
            #data[var] = ncFileWind.variables[var][0,:,6300:6800,1692:1693 ].T    
        if var in ['QVAPOR', 'QCLOUD','RE_CLOUD','QICE','RE_ICE','QNCLOUD','QNICE','QSNOW','RE_SNOW','QGRAUP','QRAIN','QNRAIN']:
            data[var] = ncFileCloud.variables[var][0,:,800:1000, 1400:1600 ].T
            #data[var] = ncFileCloud.variables[var][0,:,6300:6800,1692:1693 ].T
  ncFileWind.close()
  ncFileCloud.close()
  print("var import done")
  ...
  data["relhum"] = 100*q2rh(data['QVAPOR'],data['T'],data['P'])
  #data["relhum"][:] = 0
  data["QCLOUD"] = data["QCLOUD"][:,:,:,None]
  data["RE_CLOUD"] = data["RE_CLOUD"][:,:,:,None]
  data["QNCLOUD"] = data["QNCLOUD"][:,:,:,None]
  ...
  data["QCLOUD"][data["QCLOUD"]<=1e-9] = 0
  data["QNCLOUD"] = np.zeros(data["QCLOUD"].shape)
  #data["QCLOUD"][data["RE_CLOUD"]<=0] = 0
  data["RE_CLOUD"][data["QCLOUD"]<=1e-9] = 0


  data["QICE"] = data["QICE"][:,:,:,None]
  data["RE_ICE"] = data["RE_ICE"][:,:,:,None]
    
  #data["QICE"][:] = 0
  #data["RE_ICE"][:] = 0  
    
  data["QICE"][data["QICE"]<=1e-9] = 0
  data["QNICE"] = np.zeros(data["QICE"].shape)
  #data["QICE"][data["RE_ICE"]<=0] = 0
  data["RE_ICE"][data["QICE"]<=1e-9] = 0



  data["QSNOW"] = data["QSNOW"][:,:,:,None]
  data["RE_SNOW"] = data["RE_SNOW"][:,:,:,None]
    
  print("Shape")
  print(data["QSNOW"].shape)
  # Force snow to be lower and below cloud
  data["QSNOW_low"] = np.zeros(data["QSNOW"].shape)
  data["RE_SNOW_low"] = np.zeros(data["RE_SNOW"].shape)

  factors = np.zeros(data["QSNOW"].shape)
    
    
  """for z in np.arange(0,data["QSNOW"].shape[2]):
        data["QSNOW_low"][:,:,z//3,0] = data["QSNOW_low"][:,:,z//3,0] + data["QSNOW"][:,:,z,0]
        data["RE_SNOW_low"][:,:,z//3,0] = data["RE_SNOW_low"][:,:,z//3,0] + data["RE_SNOW"][:,:,z,0]

        newFactors = np.zeros((data["QSNOW"].shape[0],data["QSNOW"].shape[1]))
        newFactors = data["RE_SNOW"][:,:,z,0]>0
        
        factors[:,:,z//3,0] = factors[:,:,z//3,0] + newFactors*1
        print(factors)
        print(factors.shape)

  data["QSNOW"] = data["QSNOW_low"]
    
  factors[factors==0] = 1
  data["RE_SNOW"] = data["RE_SNOW_low"][:]/factors
  """
  #data["QSNOW"][:] = 0
  #data["RE_SNOW"][:] = 0

  data["QSNOW"][data["QSNOW"]<=1e-9] = 0
  data["QNSNOW"] = np.zeros(data["QSNOW"].shape)
  #data["QICE"][data["RE_ICE"]<=0] = 0
  data["RE_SNOW"][data["QSNOW"]<=1e-9] = 0


  data["QGRAUP"] = data["QGRAUP"][:,:,:,None]
  data["QNGRAUP"] = np.zeros(data["QGRAUP"].shape)
  data["RE_GRAUP"] = np.zeros(data["QGRAUP"].shape)
  data["QGRAUP"][data["QGRAUP"]<=1e-9] = 0

  #data["QGRAUP"][:] = 0
  #data["RE_GRAUP"][:] = 0
    
  data["QRAIN"] = data["QRAIN"][:,:,:,None]
  data["RE_RAIN"] = np.zeros(data["QRAIN"].shape)
  data["QNRAIN"] = data["QNRAIN"][:,:,:,None]
  data["QRAIN"][data["QRAIN"]<=1e-9] = 0
  data["QNRAIN"][data["QRAIN"]<=1e-9] = 0
    
  data["QRAIN"][:] = 0
  data["QNRAIN"][:] = 0
  
  downsample_list = ["QCLOUD","QICE","QSNOW","QGRAUP","QRAIN","RE_CLOUD","RE_ICE","RE_SNOW","RE_GRAUP","RE_RAIN","QNCLOUD","QNICE","QNSNOW","QNGRAUP","QNRAIN"]
  for var in downsample_list:
      print(var)
      print(data[var].shape)
      data[var] = downsample_3d_array(np.squeeze(data[var]),resolution, 0.15)
      data[var] = np.expand_dims(data[var], axis=3)

      
  data["hydro_q"] = np.concatenate((data["QCLOUD"],data["QICE"],data["QSNOW"],data["QGRAUP"],data["QRAIN"]),axis=3)
  data["hydro_reff"] = np.concatenate((data["RE_CLOUD"],data["RE_ICE"],data["RE_SNOW"],data["RE_GRAUP"],data["RE_RAIN"]),axis=3)
  data["hydro_n"] = np.concatenate((data["QNCLOUD"],data["QNICE"],data["QNSNOW"],data["QNGRAUP"],data["QNRAIN"]),axis=3)
  print(data["hydro_q"].shape)
  #data["QICE"] = data["QICE"][:,:,:,None]
  #data["QNCLOUD"] = data["QNCLOUD"][:,:,:,None]
 ...
 # varPairs = [["Times","timestamp"],["XLAT","lat"],["XLONG","lon"],["U10","wind10u"],["V10","wind10v"],["PH","hgt_lev"],["P","press_lev"],["T","temp_lev"],["QVAPOR","q"],["QCLOUD","cwc_q"],["QICE","iwc_q"],["QRAIN","rwc_q"],["QSNOW","swc_q"],["QGRAUP","gwc_q"]]
  varPairs = [["XLAT","lat"],["XLONG","lon"],["U10","wind10u"],["V10","wind10v"],["PH","hgt"],["P","press"],["T","temp"],["QVAPOR","q"],["hydro_q","hydro_q"],["relhum","relhum"],["hydro_reff","hydro_reff"],["TSK","groundtemp"],["hydro_n","hydro_n"]]

  downsample_list = ["PH","P","T","QVAPOR","relhum"]
  for var in downsample_list:
      print(var)
      print(data[var].shape)
      data[var] = downsample_3d_array(np.squeeze(data[var]),resolution, 0.15)
  ...
  downsample_list = ["U10","V10","XLAT","XLONG","TSK"]
  for var in downsample_list:
      print(var)
      data[var] = downsample_2d_array(data[var],resolution, 0.15)
  print(data["XLAT"])
  ...
  # surface properties
  pamData['sfc_type'] = np.zeros(data['TSK'].shape)
  pamData['sfc_sif'] = np.zeros(data['TSK'].shape)
  pamData['sfc_slf'] = np.zeros(data['TSK'].shape)
  #pamData['sfc_type'][:] = 0  #Assume all water for now #np.around(data['LANDMASK'])
  pamData['sfc_model'] = np.zeros(data['TSK'].shape)
  pamData['sfc_refl'] = np.chararray(data['TSK'].shape)
  pamData['sfc_refl'][:] = 'F'
  
  pamData['obs_height'] = np.zeros((data['TSK'].shape[0],data['TSK'].shape[1],2))
  pamData['obs_height'][:,:,0] = 833000.0 #TOA
  pamData['obs_height'][:,:,1] = 0#152.4 #flight level 500ft
    
  print("Creating pamdata")
...
def readWrfCOMBLE(fWind, fCloud, descriptorFile):
  """
...
def readWrfCOMBLERandom(fWind, fCloud, descriptorFile, slice_b, slice_e):
  """
  ...
  variables = ['XLONG', 'XLAT', 'U10', 'V10', 'TSK', 'T', 'P', 'PB', 'QVAPOR', 'QCLOUD', 'PH', 'PHB','RE_CLOUD','QICE','RE_ICE','QSNOW','RE_SNOW','QGRAUP','QRAIN','QNRAIN','QNCLOUD']
  ...
  for var in variables:
        print(var)
        if var in ['XLONG', 'XLAT','U10', 'V10', 'TSK']:
            data[var] = ncFileWind.variables[var][0,slice_b:slice_e, :].T    
        if var in ['T', 'P', 'PB', 'PH', 'PHB']:
            data[var] = ncFileWind.variables[var][0,:,slice_b:slice_e, :].T    
        if var in ['QVAPOR', 'QCLOUD','RE_CLOUD','QICE','RE_ICE','QNCLOUD','QNICE','QSNOW','RE_SNOW','QGRAUP','QRAIN','QNRAIN']:
            data[var] = ncFileCloud.variables[var][0,:,slice_b:slice_e, :].T
  ncFileWind.close()
  ncFileCloud.close()
  print("var import done")
  data["PH"] = (data["PH"]+data["PHB"])/9.81 #now "real m
  ...
  #for z in np.arange(0,data["QSNOW"].shape[0]):
        #data["QSNOW_low"][:,:,z//3,0] = data["QSNOW_low"][:,:,z//3,0] + data["QSNOW"][:,:,z,0]
        #data["RE_SNOW_low"][:,:,z//3,0] = data["RE_SNOW_low"][:,:,z//3,0] + data["RE_SNOW"][:,:,z,0]
        
  #data["QSNOW"] = data["QSNOW_low"]
  #data["RE_SNOW"] = data["RE_SNOW_low"][:]/3
  ...
  #translate variables
 # varPairs = [["Times","timestamp"],["XLAT","lat"],["XLONG","lon"],["U10","wind10u"],["V10","wind10v"],["PH","hgt_lev"],["P","press_lev"],["T","temp_lev"],["QVAPOR","q"],["QCLOUD","cwc_q"],["QICE","iwc_q"],["QRAIN","rwc_q"],["QSNOW","swc_q"],["QGRAUP","gwc_q"]]
  varPairs = [["XLAT","lat"],["XLONG","lon"],["U10","wind10u"],["V10","wind10v"],["PH","hgt"],["P","press"],["T","temp"],["QVAPOR","q"],["hydro_q","hydro_q"],["relhum","relhum"],["hydro_reff","hydro_reff"],["TSK","groundtemp"]]
  pamData = dict()
  ...
  pamData['obs_height'] = np.zeros((data['TSK'].shape[0],data['TSK'].shape[1],2))
  pamData['obs_height'][:,:,0] = 833000.0 #TOA
  pamData['obs_height'][:,:,1] = 152.4 #flight level 500ft
...
def readWrfCOMBLERandom3D(fWind, fCloud, descriptorFile, slice_b, slice_e):
  """
 ...
 # varPairs = [["Times","timestamp"],["XLAT","lat"],["XLONG","lon"],["U10","wind10u"],["V10","wind10v"],["PH","hgt_lev"],["P","press_lev"],["T","temp_lev"],["QVAPOR","q"],["QCLOUD","cwc_q"],["QICE","iwc_q"],["QRAIN","rwc_q"],["QSNOW","swc_q"],["QGRAUP","gwc_q"]]
  ...
  pamData['obs_height'] = np.zeros((data['TSK'].shape[0],data['TSK'].shape[1],14))
  pamData['obs_height'][:,:,0] = 6000 
  pamData['obs_height'][:,:,1] = 5000 #flight level 500ft
  pamData['obs_height'][:,:,2] = 4500 #flight level 500ft
  pamData['obs_height'][:,:,3] = 4000 #flight level 500ft
    
  pamData['obs_height'][:,:,4] = 3500 #flight level 500ft
  pamData['obs_height'][:,:,5] = 3000 #flight level 500ft
  pamData['obs_height'][:,:,6] = 2600 #flight level 500ft
  pamData['obs_height'][:,:,7] = 2200 #flight level 500ft
  pamData['obs_height'][:,:,8] = 1800 #flight level 500ft
  pamData['obs_height'][:,:,9] = 1400 #flight level 500ft
  pamData['obs_height'][:,:,10] = 1000 #flight level 500ft
  pamData['obs_height'][:,:,11] = 700
  pamData['obs_height'][:,:,12] = 400
  pamData['obs_height'][:,:,13] = 152.4 #flight level 500ft
...
def cloud_depth(rhs,alt):
    cloudBottom = 0
    cloudBottomInd = 0
    cloudTop = 0
    
    # make a smooth average
    kernel_size = 5
    kernel = np.ones(kernel_size)/kernel_size
    rhs = np.convolve(rhs,kernel,mode='same')
    alt = np.convolve(alt,kernel,mode='same')
    for ind, rh in enumerate(rhs):
        if rh>90:
            cloudBottom = alt[ind]
            cloudBottomInd = ind
            break
    if cloudBottom:        
        for ind in np.arange(cloudBottomInd,len(alt)):
            if rhs[ind]<85:
                cloudTop = alt[ind]
                break
    return cloudBottom,cloudTop

def readPamtraSoundingGreenland(txtInputFile, descriptorFile, q,datestr):
    """
    Parameter
    ---------
    inputFile: str
    ...
    """
    import pandas as pd
    f = pd.read_csv(txtInputFile,sep='\s+',header=0)
    
    print(f)
    pamData = dict()

    print(f["HeightMSL"][1:])
    #Number of valid altitudes
    """validAltIndexes = [
        index for index in range(len(f["HeightMSL"][1:]))
        if f["HeightMSL"][1:][index-1] > 0
    ]
    """
       
    # epoch time
    #epochTime = f2['base_time'][0]+f2['time'][0]
    #dtime = datetime.datetime.utcfromtimestamp(int(epochTime))
        

    # Over the ocean
    pamData["sfc_type"] = 0
    pamData["sfc_refl"] = 'F'
    pamData["sfc_salinity"]= 33.0
    
    
    #pamData["timestamp"] = nc.date2num(dtime,"seconds since 1970-01-01 00:00:00")
    #pamData["lat"] = f2["lat"][0]
    #pamData["lon"] = f2["lon"][0]


    ###### Modify with plane descent
    skiprows = 73
    if datestr == "20240611":
        skiprows = 72
    if datestr == "20240613":
    ...
    df = pd.read_csv('/glade/derecho/scratch/ephraims/FROM_CHEYENNE/GreenlandData/iwg/ARCSIX-MetNav_P3B_' + datestr +  '_RA.ict',skiprows=skiprows)
    ...
    for ind in df.index[::-1]:
        if df["GPS_Altitude"][ind]>210:
            break

        if (df["GPS_Altitude"][ind]>90) & (df["GPS_Altitude"][ind]>altPrev):
            altPrev = pd.to_numeric(df["GPS_Altitude"][ind])
            planeAlt.append(pd.to_numeric(df["GPS_Altitude"][ind]))
            planeRH.append(pd.to_numeric(df["Relative_Humidity"][ind]))
            planeTemp.append(pd.to_numeric(df["Static_Air_Temp"][ind])+273.15)
            planePress.append(pd.to_numeric(df["Static_Pressure"][ind])*100)

    
    pamData["hgt"] = np.concatenate((planeAlt,pd.to_numeric(f['HeightMSL'][1::3]).to_numpy()))
    pamData["press"] = np.concatenate((planePress,(pd.to_numeric(f['P'][1::3])*100).to_numpy()))
    pamData["temp"] = np.concatenate((planeTemp,(pd.to_numeric(f['Temp'][1::3])+273.15).to_numpy()))
    pamData["relhum"] = np.concatenate((planeRH,pd.to_numeric(f['RH'][1::3]).to_numpy()))

    print(pamData["hgt"])
    print(pamData["press"])
    print(pamData["temp"])
    print(pamData["relhum"])

    #pamData["relhum"][pamData["hgt"]>4000] = pamData["relhum"][pamData["hgt"]>4000]+2
    #pamData["relhum"][pamData["hgt"]<4000] = pamData["relhum"][pamData["hgt"]<4000]+2
    pamData["relhum"][pamData["relhum"]>100] = 100
    ...
    # Linear distribution of cloud water
    cwc_q = np.zeros((len(pamData["hgt"]),1))
    cwc_reff = np.zeros((len(pamData["hgt"]),1))
    reff = 20*10e-6


    
    pamData['obs_height'] = np.zeros((1,1,2))
    pamData['obs_height'][:,:,0] = 833000.0 #TOA
    pamData['obs_height'][:,:,1] = pamData["hgt"][0] + q #flight level 500ft
    
    print(pamData["hgt"][0])
    
    """zbottom, ztop = cloud_depth(pamData["relhum"],pamData["hgt"])
    
    #zbottom = 500
    #ztop = 2000
    lwp = q/1000 #in kg
    qmax = (lwp/(ztop-zbottom))*2
    Rair = 287.04  # J/kg/K
    
    indBottom = find_nearest_idx(f2['alt'][validAltIndexes],zbottom)
    indTop = find_nearest_idx(f2['alt'][validAltIndexes],ztop)
    for ind in np.arange(0,indTop-indBottom):
        cwc_q[ind+indBottom] = (qmax*(ind/(indTop-indBottom)))/((pamData["press"][ind+indBottom]/(Rair*pamData["temp"][ind+indBottom])))
    
    
    cwc_reff[cwc_q>0] = reff
    """
    pamData["hydro_q"] = cwc_q
    pamData["hydro_reff"] = cwc_reff
    #pamData["hydro_n"] = np.ones((len(validAltIndexes),1))*1000
...
def readPamtraSoundingCAESAR(ncInputFile, descriptorFile, alt, date):
    """
    Parameter
    ---------
    inputFile: str
    ...
    pamData = dict()
    f = nc.Dataset(ncInputFile)
    
    """#Number of valid altitudes
    validAltIndexes = [
        index for index in range(len(f2['alt'][:]))
        if f2['alt'][:][index] > 0
    ]
    """
    ind = 0
    for time in f["time"][:]:
        soundingTime = datetime.datetime.fromtimestamp(time)
        print(soundingTime)
        print(date)
        if soundingTime == date:
            break
        ind = ind + 1

    #Number of valid altitudes
    validAltIndexes = [
        index for index in range(len(f['altitude'][ind,:]))
        if f['altitude'][ind,:][index] > 0
    ]
    ...
    pamData["hgt"] = f['altitude'][ind,validAltIndexes]
    pamData["press"] = f['air_pressure'][ind,validAltIndexes]*100
    pamData["temp"] = f['air_temperature'][ind,validAltIndexes]
    pamData["relhum"] = f['relative_humidity'][ind,validAltIndexes]
    ...
    # Linear distribution of cloud water
    cwc_q = np.zeros((len(pamData["relhum"]),1))
    cwc_reff = np.zeros((len(pamData["relhum"]),1))
    reff = 20*10e-6
    
    pamData['obs_height'] = np.zeros((1,1,2))
    pamData['obs_height'][:,:,0] = 833000.0 #TOA
    pamData['obs_height'][:,:,1] = alt #flight level 500ft
...
def readPamtraSounding(ncInputFile, descriptorFile, q):
    """
    Parameter
    ---------
    inputFile: str
    ...
    pamData["hgt"] = f2['alt'][validAltIndexes[::3]]
    pamData["press"] = f2['pres'][validAltIndexes[::3]]*100
    pamData["temp"] = f2['tdry'][validAltIndexes[::3]]+273.15
    pamData["relhum"] = f2['rh'][validAltIndexes[::3]]
    ...
    # Linear distribution of cloud water
    cwc_q = np.zeros((len(validAltIndexes[::3]),1))
    cwc_reff = np.zeros((len(validAltIndexes[::3]),1))
    reff = 20*10e-6
...
def readPamtraSoundingTxt(inputFile, descriptorFile, q):
    """
    Parameter
    ---------
    inputFile: str
    ...
    import pandas as pd
    df = pd.read_csv(inputFile, sep=",",skipinitialspace=True) 
    ...
    pamData['obs_height'] = np.zeros((1,1,2))
    pamData['obs_height'][:,:,0] = 833000.0 #TOA
    pamData['obs_height'][:,:,1] = 317 #flight level 500ft
    ...
    #rh = relative_humidity_from_dewpoint(df['TEMP'][:] * units.degC, df['DWPT'][:] * units.degC).to('percent')
    rh = np.zeros(len(df))
    
    for i in np.arange(0,len(df)):
        rh[i] = relative_humidity_from_dewpoint(df['TEMP'][i] * units.degC, df['DWPT'][i] * units.degC).to('percent').magnitude
    
    
    
    pamData["hgt"] = df['HGHT'][:]
    pamData["press"] = df['LEVEL'][:]*100
    pamData["temp"] = df['TEMP'][:]+273.15
    pamData["relhum"] = rh
    pam = pyPamtra()
    for hyd in descriptorFile: pam.df.addHydrometeor(hyd)
    ...
    # Linear distribution of cloud water
    cwc_q = np.zeros((len(df),1))
    cwc_reff = np.zeros((len(df),1))
    """reff = 20*10e-6
